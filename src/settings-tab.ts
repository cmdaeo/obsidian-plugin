...
export class GitSyncSettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .addText((t) =>
        t.setPlaceholder("main")
          ...
          .onChange(async (v) => {
            this.plugin.settings.branchName = v.trim() || "main";
            await this.plugin.saveSettings();
          })
      );

    // ── GitHub OAuth App ──────────────────────────────────────────────────
    containerEl.createEl("h2", { text: "GitHub OAuth App" });
    ...
    new Setting(containerEl)
      ...
      .addText((t) =>
        t.setPlaceholder("Ov23li…")
          ...
          .onChange(async (v) => {
            this.plugin.settings.githubClientId = v.trim();
            await this.plugin.saveSettings();
          })
      );
    ...
    new Setting(containerEl)
      ...
      .addText((t) => {
        ...
        t.setPlaceholder("••••••••••••••••••••••••••••••••••••••••")
          ...
          .onChange(async (v) => {
            this.plugin.settings.githubClientSecret = v.trim();
            await this.plugin.saveSettings();
          });
      });
      ...
      new Setting(containerEl)
        ...
        .addButton((btn) =>
          btn.setButtonText("Connect with GitHub").setCta().onClick(async () => {
            ...
            if (!id || !secret) {
              new Notice("Git Sync: Enter your GitHub Client ID and Secret first.");
              return;
            }
            ...
            if (session) {
              ...
              await this.plugin.saveSettings();
              new Notice(`Git Sync: Connected as @${session.username}`);
              this.display();
            }
          })
        );
    ...
    // ── GitLab OAuth App ──────────────────────────────────────────────────
    containerEl.createEl("h2", { text: "GitLab OAuth App" });
    containerEl.createEl("p", {
      text: "Register your own app at gitlab.com/-/profile/applications. " +
            "Scopes: read_user, read_repository, write_repository. " +
            "Redirect URI: obsidian://git-sync-callback",
      cls: "setting-item-description",
    });
    ...
    new Setting(containerEl)
      ...
      .addText((t) =>
        t.setPlaceholder("abc123…")
          ...
          .onChange(async (v) => {
            this.plugin.settings.gitlabClientId = v.trim();
            await this.plugin.saveSettings();
          })
      );
    ...
    new Setting(containerEl)
      ...
      .addText((t) => {
        ...
        t.setPlaceholder("••••••••••••••••••••••••••••••••••••••••")
          ...
          .onChange(async (v) => {
            this.plugin.settings.gitlabClientSecret = v.trim();
            await this.plugin.saveSettings();
          });
      });
      ...
      new Setting(containerEl)
        ...
        .addButton((btn) =>
          btn.setButtonText("Connect with GitLab").setCta().onClick(async () => {
            ...
            if (!id || !secret) {
              new Notice("Git Sync: Enter your GitLab Application ID and Secret first.");
              return;
            }
    ...
    // ── Gitea ─────────────────────────────────────────────────────────────
    containerEl.createEl("h2", { text: "Gitea / Self-hosted" });
        ...
        .addButton((btn) =>
          btn.setButtonText("Connect Gitea instance…").onClick(() => {
            new GiteaPATModal(this.app, async (result) => {
              if (!result) return;
              try {
                const res = await fetch(`${result.baseUrl}/api/v1/user`, {
    ...
    new Setting(containerEl)
      ...
      .setDesc("Debounce-commit and push after every vault modification.")
      .addToggle((t) =>
      ...
      .addText((t) =>
        t.setPlaceholder("5000")
          ...
          .onChange(async (v) => {
            ...
            if (!isNaN(ms) && ms >= 500) {
              this.plugin.settings.autoSyncDebounceMs = ms;
              await this.plugin.saveSettings();
            }
          })
      );
    ...
    new Setting(containerEl)
      ...
      .addToggle((t) =>
        t.setValue(this.plugin.settings.auditLogEnabled).onChange(async (v) => {
          this.plugin.settings.auditLogEnabled = v;
          await this.plugin.saveSettings();
        })
      );
    ...
    new Setting(containerEl)
      ...
      .addText((t) =>
        t.setPlaceholder("_System/Sync_Log.md")
          .setValue(this.plugin.settings.auditLogPath)
          .onChange(async (v) => {
            this.plugin.settings.auditLogPath = v.trim() || "_System/Sync_Log.md";
            await this.plugin.saveSettings();
          })
      );
  }

  private renderConnectedBanner(containerEl: HTMLElement, provider: string) {
    const s = this.plugin.settings.session!;
    const labels: Record<string, string> = { github: "GitHub", gitlab: "GitLab", gitea: "Gitea" };

    const setting = new Setting(containerEl)
      .setName(`✓ Connected to ${labels[provider]}`)
      .setDesc(`@${s.username}${s.email ? ` · ${s.email}` : ""} — authorized ${formatRelative(s.authorizedAt)}`);

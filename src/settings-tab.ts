...
export class GitSyncSettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    containerEl.createEl('p', {
      text: 'Register your own app at github.com/settings/developers → OAuth Apps → New. Set the callback URL to obsidian://git-sync-callback',
    ...
    if (this.plugin.settings.session?.provider === 'github') {
      ...
      new Setting(containerEl)
        .setName('Connect')
        .addButton(btn => btn
          .setButtonText('Connect with GitHub')
          .setCta()
          .onClick(async () => {
    ...
    if (this.plugin.settings.session?.provider === 'gitlab') {
      ...
      new Setting(containerEl)
        .setName('Connect')
        .addButton(btn => btn
          .setButtonText('Connect with GitLab')
          .setCta()
          .onClick(async () => {
    ...
    if (this.plugin.settings.session?.provider === 'gitea') {
      ...
      new Setting(containerEl)
        .setName('Connect')
        .addButton(btn => btn
          .setButtonText('Connect Gitea instance')
          .onClick(() => new GiteaPATModal(this.app, async result => {
            if (!result) return;
            try {
    ...
    new Setting(containerEl)
      .setName('Auto-sync')
      .setDesc('Automatically commit and push after changes (debounced)')
      .addToggle(t => t
    ...
    new Setting(containerEl)
      ...
      .addText(t => t
        ...
        .onChange(async v => {
          const n = parseInt(v);
          if (!isNaN(n) && n >= 1000) {
            this.plugin.settings.autoSyncDebounceMs = n;
            await this.plugin.saveSettings();
          }
        }));
    ...
    new Setting(containerEl)
      ...
      .addText(t => t
        .setPlaceholder('_System/SyncLog.md')
        .setValue(this.plugin.settings.auditLogPath)
        .onChange(async v => {
          this.plugin.settings.auditLogPath = v.trim();
          await this.plugin.saveSettings();
        }));
  }
  ...
  private renderConnectedBanner(containerEl: HTMLElement, provider: string): void {
    ...
    const label = provider === 'github' ? 'GitHub' : provider === 'gitlab' ? 'GitLab' : 'Gitea';
    new Setting(containerEl)
      .setName(`Connected to ${label}`)
